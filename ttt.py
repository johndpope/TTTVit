...
class TTTCache:
    ...
    def __init__(self, model, batch_size: int):
        ...
        if hasattr(config, 'ttt_layer_type'):
            ...
            # Default to linear if ttt_layer_type is not specified
            self.ttt_param_names = ["W1", "b1"]
        ...
        for layer_idx in range(config.num_hidden_layers):
            for name in self.ttt_param_names:
                weight = getattr(model.blocks[layer_idx].ttt, name)
            ...
            if hasattr(config, 'pre_conv') and config.pre_conv:
            ...
            if hasattr(config, 'share_qk') and config.share_qk:
